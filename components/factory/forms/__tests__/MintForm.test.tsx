...
describe('MintForm Component', () => {
  ...
  test('mint button is enabled when inputs are valid', () => {
    renderWithChainProvider(<MintForm {...mockProps} />);
    fireEvent.change(screen.getByLabelText('mint-amount-input'), { target: { value: '100' } });
    fireEvent.change(screen.getByLabelText('mint-recipient-input'), { target: { value: 'cosmos1recipient' } });
    const mintButton = screen.getByText('Mint');
