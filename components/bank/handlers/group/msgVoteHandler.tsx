...
export const MsgVoteHandler = createSenderReceiverHandler({
  ...
  successSender: tx =>
    `You voted ${formatVote(tx.metadata?.option)} on proposal #${tx.proposal_ids}`, // TODO Link to proposal
  failSender: tx => `You failed to vote on proposal #${tx.proposal_ids}`, // TODO Link to proposal
  successReceiver: tx => `Proposal #${tx.proposal_ids} was voted on by ${tx.sender}`,
});
