...
export const MsgCreateGroupWithPolicyHandler = createSenderReceiverHandler({
  ...
  successSender: tx =>
    createMessage('You created a new group named {0}', tx.metadata?.groupMetadata),
  failSender: tx =>
    createMessage('You failed to create a new group named {0}', tx.metadata?.groupMetadata),
  successReceiver: tx =>
    createMessage('You were mentioned in a new group named {1}', tx.metadata?.groupMetadata),
});
