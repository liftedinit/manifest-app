  ...
  return (
    <div className="fixed inset-0 pointer-events-none flex items-end justify-end p-4 z-[9999]">
      <div className="toast toast-end toast-bottom flex flex-col justify-start items-center text-left pointer-events-auto">
        <div className="toast toast-end toast-bottom flex flex-col justify-start items-center text-left">
          <div
            ref={toastRef}
            className={`alert ${toastMessage.type} w-96 h-full relative
    transition-all duration-300 ease-in-out
    ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}
    ${prevMessage && prevMessage.type !== toastMessage.type ? 'animate-pulse' : ''}
    bg-white dark:bg-[#161616] border-[0.5px] ${getBorderColor(toastMessage.type)}
    text-black dark:text-white shadow-lg rounded-lg overflow-hidden`}
          >
            ...
            <button
              type="button"
              className="p-2 text-[#161616] dark:text-white bg-neutral rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 absolute top-2 right-2 transition-colors duration-200 z-10"
              onClick={handleClose}
            >
            ...
            <div className="flex flex-col w-full h-full overflow-hidden">
              ...
              {toastMessage.type === 'alert-error' && (
                <button
                  type="button"
                  id="copyButton3"
                  className="p-1 text-[#161616] dark:text-white absolute bottom-2 right-3 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-200"
                  onClick={() => {
