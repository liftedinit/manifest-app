...
export const getCosmostationUrl = (
  ...
  const dynamicChainName =
    env === "testnet" ? `${convertedChainName}-testnet` : convertedChainName;
...
export const addLogoUrlSource = async (
  ...
  const url = getCosmostationUrl(chainName, validator.operator_address);
  const isValid = await isUrlValid(url);
  return {
    ...validator,
    imageSource: isValid ? "cosmostation" : "keybase",
  };
};
